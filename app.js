(function(){
  ...
  function persistAllVisibleRows(user){
    var key=currentYM();
    var tb=$('rows'); if(!tb) return Promise.resolve();
    var ps=[]; var trs=tb.querySelectorAll('tr[id^="row-"]');
    for(var i=0;i<trs.length;i++){ var ds=trs[i].id.replace('row-',''); 
      var date=new Date(ds);
      // We can't reconstruct habitual easily here without cfg; but we'll rely on applyStateToUI calling persist later; skip here.
    }
  ...
  // ===== No habitual: no resetea tabla (persistimos primero) =====
  $('nh-save').addEventListener('click', function(){
    ...
    }).then(function(){ setMsg($('nh-msg'),'Extra guardada',true); $('last-update').textContent=new Date().toLocaleString(); })
      .then(function(){ return paintCurrentUser(); });
  });
  ...
  firebase.auth().onAuthStateChanged(function(user){
    ...
    getRole(user.uid).then(function(role){
      ...
      if(role==='admin'){
        $('view-switch').classList.remove('hidden'); // admins pueden ver el switch
        $('employee-view').classList.add('hidden'); $('admin-view').classList.remove('hidden');
        $('to-admin').classList.remove('ghost'); $('to-employee').classList.add('ghost');
      }else{
        $('view-switch').classList.add('hidden'); // empleados NO ven admin
        $('employee-view').classList.remove('hidden'); $('admin-view').classList.add('hidden');
      }
      paintTable(user);
    });
  ...
  // Exponer doLogin por si el inline no carga
